...
5. Run `ruff format --check src tests`, `ruff check src tests`, `python -m pytest`, `python -m compileall -q src tests`, `studio framework validate`, `studio validate`, `studio report`, `studio status`, `studio path check`, and `git diff --check`.
