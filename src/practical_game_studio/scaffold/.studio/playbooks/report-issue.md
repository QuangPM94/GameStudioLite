...
Use for `/report-issue` whenever a specific, actionable problem is identified. Use one invocation per distinct underlying cause, not per symptom.
...
9. Do not mark the issue `--on-critical-path` or otherwise place it on the critical path automatically; that determination belongs to `/critical-path`.
...
`/record-evidence` when the report includes supporting evidence; `/critical-path` only when the user asks whether the issue affects the milestone path; otherwise `/next-step`.
